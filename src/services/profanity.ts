/**
 * Mirrors sttm-server `src/utils/profanity.js` and sttm-admin
 * `src/utils/profanity.ts` — this copy is UX, the server one is the guard.
 *
...
const fold = (str: string): string => {
  ...
  out = out.replace(/[\u0400-\u04ff]/g, (char) => (char in CYRILLIC ? CYRILLIC[char] : char));
 ...
 * @private
 */
const expand = (strs: string[], letter: string, readings: string[]): string[] => {
...
const foldedRoots = [...new Set(ROOTS.map(fold).filter((root) => root.length >= MIN_ROOT_LENGTH))];
