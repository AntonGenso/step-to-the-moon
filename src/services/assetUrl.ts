 ...
 * MinIO (`http://host:9000/…`). The direct form is plain HTTP, and the game is
 * served over HTTPS in production — the browser then blocks the image as mixed
 * content, which is why covers and facts came up empty there.
 *
 * So an insecure absolute link is folded onto the same-origin path this app
 * already proxies (see the `/uploads` rewrite in `next.config.mjs`). Links that
 * are relative, or absolute over HTTPS, are left exactly as they are.
 *
 * Mirrored in sttm-admin `src/utils/assetUrl.ts`.
 */

/** Path the app proxies to MinIO; must match the rewrite in next.config.mjs. */
